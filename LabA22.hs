...
mMergeSort xs = runPar $ mMergeSort_ xs

mMergeSort_ :: (Ord a, NFData a) => [a] -> Par [a]
mMergeSort_ xs =
      case Main.split xs of
        ([], [])       -> return []
        (x:[], [])     -> return [x]
        (sp1, sp2) -> if ((length sp1) < 500) then
                              return $ merge (mergeSort sp1) (mergeSort sp2)
                                              else do
              i <- spawn (mMergeSort_ sp1)
              j <- spawn (mMergeSort_ sp2)
              rs1 <- get i
              rs2 <- get j
